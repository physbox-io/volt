 ...
 *
 * Keyed on the exact node type rather than substring matching, because types
 * like 'npn', 'opamp' and 'sevenseg' contain none of the words a substring
 * check would look for and would otherwise silently fall back to a two-pad
 * 0805 — losing every pin past the second.
 */
...
export function resolveFootprint(packageId?: string, componentType?: string, pinCount = 2): ComponentFootprint {
  ...
  if (packageId) {
    ...
    if (packageId.startsWith('HEADER-')) {
