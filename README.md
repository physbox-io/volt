...
```bash
docker build --build-arg GITHUB_TOKEN=$(gh auth token) -t physbox-volt .
docker run -p 8000:8000 physbox-volt
```

### GitHub Packages dependency (`@physbox-io/ui`)

Volt depends on the shared `@physbox-io/ui` design-token package, published to
...
Pushes to `main` deploy automatically via the `physbox-volt-deploy` Cloud
Build trigger (service `circuitsim`, region `us-west1`; see internal infra
notes for the GCP project). This trigger's build steps are **not** a
`cloudbuild.yaml` in this repo — they were auto-generated by GCP's "Deploy to
Cloud Run" flow and live only in the trigger config. To view or edit them:

```bash
gcloud builds triggers describe physbox-volt-deploy --project=<gcp-project-id>
...
to `read:packages`, shared with Etch's equivalent trigger), via the trigger's
`availableSecrets`/`secretEnv`. Because Cloud Build's `docker` step args are
NOT shell-expanded by default, the step invokes `docker build` through
`sh -c "..."` so `$$GITHUB_TOKEN` actually expands to the secret's value —
passing it as a literal step arg silently sends the string `$GITHUB_TOKEN`
instead of the token. The build also passes `--network=cloudbuild` to
`docker build`.
