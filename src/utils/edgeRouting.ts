 ...
 * another without crossing anything: the A* router, its obstacle map and the
 * passes that tidy the result.
 *
 * All of this lived in `AuraEdge.tsx` beside the edge component, where it cost
 * the canvas its Fast Refresh — a module that exports anything but components
 * is reloaded whole, and a full reload here throws away the running simulation
 * and the view the user was part-way through editing. It also put the router on
 * both sides of an import cycle: `nodeGeometry.ts` needs the part dimensions,
 * and the router needs `nodeGeometry`'s handle coordinates.
 */
...
export function pathNearPoint(pts: Point[], p: Point, tol: number): boolean {
  for (let i = 0; i < pts.length - 1; i++) {
    const a = pts[i];
    const b = pts[i + 1];
    if (
...
function clampToPin(nodes: Node[], x: number, y: number, pos: string, id?: string): Point {
  ...
  if (!w || !h) return { x, y };
  // Pull an endpoint that overshoots back onto the pin. Endpoints that sit
  // inside their node are left alone: the instrument cards attach inside their
  // own box deliberately, and forcing those out to the border moves the wire
  // off the lead it is drawn to.
...
export function getOrthogonalPathThroughWaypoint(
  sourceX: number,
  sourceY: number,
  sourcePosition: string,
  targetX: number,
